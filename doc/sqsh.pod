...
See also the B<$max_timeout> variable, which controls the number of times a timeout 
needs to be retried before the query is aborted.
...
Kerberos support: Specifies a I<server principal> to use  for  network  (kerberos) authentication, if
the servername in the interfaces file differs from the real servername. 
...
specifies the number of seconds I<sqsh> will wait before timing out a
login request. Maps to the B<$login_timeout> variable.

=item -U username

The Sybase I<username> to connect to the database as,  this
defaults  to the username of the user running I<sqsh>.  The
...
the version of CT-Lib used to compile I<sqsh> does not support  this  option, then it will be ignored. This option
...
by command line options). Following that,  if  I<sqsh>  is  run  in
...
When a line is first read by I<sqsh>, the first word is separated from the
...
to see if it is either a valid I<sqsh> command or alias.
...
After  a  line of input has been read, I<sqsh> attempts to expand the line
...
Typically  very few I<sqsh> commands actually read anything
...
Causes  the  function to be exported as a I<sqsh> command.  That
...
Starting with 2.1.6 sqsh provides the same command line options
...
server principal name when Servername specified in interfaces differs 
from the real servername
...
Requested security mechanism name defined for Kerberos, DCE or PAM in your libtcl.cfg file. 
...
Remove a history buffer from the history list.
...
If set, I<sqsh> maintains a MRU-LRU order of executed buffers and does not store duplicate
...
I<sp_who> is the last executed command and the buffer - originally the last in the list -
...
By default, I<sqsh> terminates if the user presses ^d (control-D) on
an empty line and readline support is compiled in. If B<$ignoreeof> is set in 
the sqshrc file you get a warning message instead:
...
to a data server. Default is 0 (infinite wait). See also the B<-T>
command line option.
...
before it is aborted. See B<query_timeout> and the B<-Q> command line flag.
...
reached the query is aborted (canceled). See also B<$max_timeout> and
the B<-Q> command line option.
=item rcfile (path)
...
Color codes are presented in a string between {1;2;3}. If sqsh encounters a {
(curly brace) in the prompt string it will assume a colorcode is supplied 
and will act as such. No checks will be performed on the validity of the colorcode
...
The third value defines the Background Color Code: 40=black 41=red 42=green 43=yellow 44=blue 45=magenta 46=cyan 47=white
...
Note, colorsupport is automatically available if sqsh is compiled and linked
